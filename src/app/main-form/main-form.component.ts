...
export class MainFormComponent implements OnInit {
    ...
    onSubmitMake(event: Event) {
        if (this.make_year_form.invalid) {
            return;
        }
        this.final_order.make = this.make_year_form.controls['make'].value;
        this.final_order.year = this.make_year_form.controls['year'].value
        this.vehicleService.getModelOptions(this.final_order.make, this.final_order.year).pipe(first())
            .subscribe(
                data => {
    ...
    onSubmitModel(event: Event) {
        if (this.model_form.invalid) {
            return;
        }
        this.final_order.model = this.model_form.controls['model'].value
        this.vehicleService.getTrimOptions(this.final_order.make, this.final_order.model, this.final_order.year)
            .subscribe(
                data => {
                    this.progress_value = 35;
                    this.trim_options = data;
                },
                error => {
                    this.alertService.subject.next("Something went wrong...");
                });
    ...
    onSubmitTrim(event: Event) {
        if (this.trim_form.invalid) {
            return;
        }
        this.final_order.trim = this.trim_form.controls['trim'].value
        console.log(this.trim_form.controls['trim'].value);
        console.log("jjfoeruniurngor");
        this.vehicleService.getTireSize(this.final_order.make, this.final_order.model, this.final_order.year, this.final_order.trim)
            .subscribe(
                data => {
                    console.log(data);
                    this.progress_value = 50;
                    this.tire_size_options = data;
                },
                error => {
                    this.alertService.subject.next("Something went wrong...");
                });
    ...
    onSubmitType(event: Event) {
        if (this.trim_form.invalid) {
            return;
        }
        this.progress_value = 80;
        this.final_order.type = this.type_form.controls['type'].value
    }
    ...
    onSubmitSize(event: Event) {
        ...
        this.final_order.size = this.tire_size_options[0];
    }
    ...
    onSubmitExtra(event: Event) {
        console.log(event);
        if (this.extra_info_form.invalid) {
            return;
        }
    ...
    updateMatchCheck() {
        console.log(this.final_order.choices.length)
        console.log(this.final_order.amount);
    ...
    /** Verifies that the tire size is correct and we can move on */
    confirmSize() {
        this.size_confirm = false;
    }
    ...
    initForms() {
        ...
        this.tire_size_form = this.fb.group({
            tire_confirm: new FormControl('', [])
        })
        this.tire_choice_form = this.fb.group({
            tire_amount: new FormControl('', [Validators.required, lengthValidator(this.final_order.choices.length, this.final_order.amount)]),
        });
    }
    ...
    initListeners() {
        ...
        this.make_year_form.valueChanges.subscribe(x => {
            if (x['year'] != "" && x['make'] != "") {
                this.goForward();
                this.onSubmitMake(null);
            }
        })
        this.model_form.valueChanges.subscribe(x => {
            if (x['model'] != "") {
                this.goForward();
                this.onSubmitModel(null);
            }
        })
        this.trim_form.valueChanges.subscribe(x => {
            if (x['trim'] != "") {
                this.goForward();
                this.onSubmitTrim(null);
            }
        })
        ...
        this.tire_choice_form.valueChanges.subscribe(x => {
            if (x['tire_amount'] === 4) {
                this.check_all_options()
            }
            else {
                this.final_order.amount = Number(x['tire_amount']);
            }
